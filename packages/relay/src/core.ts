...
/**
 * Guess limit for connect-code claims, PER CONNECTION — a decided property,
 * not an oversight, and the arithmetic is here so nobody strengthens the
 * wrong thing.
 *
 * The counter was never what stands between a guesser and someone's agent.
 * The alphabet is 32 symbols over 8 characters (32^8 = 1.1e12) and a code
 * lives for CONNECT_TTL_MS. A guesser who reconnects freely gets 20 tries per
 * handshake, and each handshake costs a fresh keypair, a signature and a round
 * trip:
 *
 *     100 reconnects/s -> 3.6e5 guesses in the window -> p = 3.3e-07
 *    1000 reconnects/s -> 3.6e6                        -> p = 3.3e-06
 *   10000 reconnects/s -> 3.6e7                        -> p = 3.3e-05
 *
 * The entropy and the TTL do the work. This counter stops cheap in-band
 * grinding on one socket, which is worth having and is all it claims.
 *
 * The tempting "fix" is a per-agent or per-IP limit. That would add durable
 * state to a relay whose entire design is that it holds none (ADR-016), to
 * move 3.3e-05 to something imperceptibly smaller. If any term here deserves
 * attention it is the TTL, since the probability is linear in it — and three
 * minutes is already bounded by a human reading a code aloud rather than by
 * the guesser.
 */
...
/** Same rule for a revoke: the ack is machine-speed or it is not coming. */
const PENDING_REVOKE_TTL_MS = 30 * 1000;

/**
 * One in-flight revoke per (agent, origin). The separator cannot occur in a
 * pubkey, so no origin string can collide its way into another agent's slot.
 */
function revokeKey(agent: string, origin: string): string {
  return `${agent} ${origin}`;
}
/**
...
export class RelayCore {
  ...
  /** In-flight revokes, so the agent's answer reaches the owner who asked. */
  #pendingRevokes = new Map<string, { conn: Conn; agent: Conn; at: number }>();
  #log: Logger;
  #now: () => number;
  ...
  close(peer: Peer): void {
    ...
    for (const [key, revoke] of this.#pendingRevokes) {
      if (revoke.conn === conn || revoke.agent === conn) this.#pendingRevokes.delete(key);
    }
  ...
  #onFrame(conn: Conn, frame: Frame): void {
    ...
    switch (frame.t) {
      ...
      case 'revoke':
        return this.#revoke(conn, frame);
      case 'revoked':
        return this.#revoked(conn, frame);
      default:
  ...
  /**
   * Withdrawing authority is an OWNER act (ADR-022 R8). Unlike `session.open`,
   * a signed delegation is not enough: the connection must itself be the key
   * the agent's cert names as owner. A page holding a delegation can close its
   * own session and nothing else — it must never be able to end another
   * origin's attachment, or to end its own in a way the user did not choose.
   */
  #revoke(conn: Conn, frame: Extract<Frame, { t: 'revoke' }>): void {
    if (conn.role !== 'client') return this.#fail(conn, 'role', 'only clients revoke');

    const agentConn = this.#agents.get(frame.agent);
    if (!agentConn || agentConn.cert?.user !== conn.pubkey) {
      // One answer for offline and not-yours: a client that does not own this
      // agent must not learn whether it exists (invariant: unowned agents are
      // invisible, proven in e2e section 7).
      return this.#fail(conn, 'forbidden', 'not your agent');
    }

    this.#pendingRevokes.set(revokeKey(frame.agent, frame.origin), {
      conn,
      agent: agentConn,
      at: this.#now(),
    });
  ...
  /** Route the agent's answer back to whoever asked, and only to them. */
  #revoked(conn: Conn, frame: Extract<Frame, { t: 'revoked' }>): void {
    if (conn.role !== 'agent') return this.#fail(conn, 'role', 'only agents answer a revoke');

    const key = revokeKey(frame.agent, frame.origin);
    const waiting = this.#pendingRevokes.get(key);
    if (!waiting || waiting.agent !== conn) return;
    this.#pendingRevokes.delete(key);
    if (this.#now() - waiting.at > PENDING_REVOKE_TTL_MS) return;
    if (this.#conns.has(waiting.conn)) waiting.conn.peer.send(frame);
  }
