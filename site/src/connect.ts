...
import { generateKeyPair, randomId } from '@agentport/protocol';
  ...
  return new Promise<WalletConnectResult>((resolve, reject) => {
    ...
    const send = () => {
      ...
      try {
        ...
        console.info('[agentport] wallet popup is not ready yet', err);
      }
    };
...
async function connectWithoutExtension(request: AgentConnectRequest, preopened?: Window | null): Promise<AgentSession> {
  ...
  try {
    ...
    console.info(`[agentport] hosted wallet ${err.reason}; using the connect-code fallback`);
